...
Российские тестовые данные для PHP: ФИО и реквизиты организаций и банков — ИНН, ОГРН и ОГРНИП,
КПП, БИК, корреспондентский и расчётный счёт.

Данные выдаются **согласованными между собой**, и в этом вся разница с генератором строк нужной
длины: фамилия, имя и отчество согласованы по роду; ИНН, ОГРН и КПП одной организации принадлежат
одному региону и одной налоговой инспекции; у ИП не бывает КПП, зато есть ФИО; контрольный ключ
счёта вычисляется от БИК того банка, где счёт открыт.
...
$person->gender;          // RuFaker\Enum\Gender::Male
$person->gender->title(); // 'мужской'
$person->full();          // 'Поляков Валентин Арсеньевич'
...
Пол задаётся явно — от него зависят все три части имени:

```php
use RuFaker\Enum\Gender;

$ru->person(Gender::Female)->full();  // 'Романова Василиса Львовна'
```

Форма и регион организации задаются так же:
...
$entrepreneur->person?->full(); // 'Успенский Пётр Иванович' — у юрлица здесь null
```
...
ФИО — `Result\Person`: фамилия, имя, отчество и пол `Enum\Gender`. Согласованы все три части,
включая отчества, образованные не по общему правилу (`Ильинична`, `Кузьминична`), и несклоняемые
фамилии (`Шевченко`). Списки собственные, около трёхсот позиций; сторонние справочники не
подключаются.
...
всё остальное: длину ИНН и ОГРН, наличие КПП, балансовый счёт. Балансовый счёт берётся из Плана
счетов Положения Банка России от 24.11.2022 № 809-П: `40702` «Коммерческие организации» для
юридического лица, `40802` «Индивидуальные предприниматели» для ИП.

Готовые наборы — `Result\Organization`, `Result\BankAccount` и `Result\Person`: у каждого есть
`toArray()`, и `json_encode()` возвращает то же самое. Это их общий контракт `Result\Result`.
Конструкторы отвергают противоречивые данные, так что собрать вручную ИП с КПП, ИП без ФИО или
счёт, не соответствующий своему банку, не получится.

Любой реквизит приводится к строке и сериализуется в JSON.
...
- Наименований организаций, адресов и банковских названий пакет не генерирует.
